function [param,stat,chance_point]=sigm_fit_color(x,y,fixed_params,initial_params,plot_flag,col)
...
%if plot_flag==1 
    ...
    x_vector=-10:0.1:18;
    plot(x_vector,f(param(isnan(fixed_params)),x_vector),  'Color',col,'LineWidth',2); hold on
   % plot(x,y,'o','Color',col,'LineWidth',2,'MarkerFaceColor','w')
    xlim([min(x) max(x)])
    chance_point = x_vector(find(f(param(isnan(fixed_params)),x_vector)>0.5,1));
