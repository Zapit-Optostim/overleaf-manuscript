...
Beam moving back and forth by 10 mm along one axis over the surafce of a ThorLab's photodiode. The setup uses ScannerMax galvos. The beam is blanked during the motion epoch (which is about 0.4 ms). We the same NI DAQ to acquire the galvo command signal, the position feedback signal from the galvo, and the photodiode. The idea being that we can overlay the galvo angular position and the photodiode trace and demonstrate that the beam is off during the period the scanner is moving. We acquire some 300 or so back and forth cycles of the beam so can plot the beam going left to right and also right to left. 
